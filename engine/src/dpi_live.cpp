...
public:
    ...
    void handle_live_packet(const struct pcap_pkthdr* pkthdr, const u_char* packet) {
        RawPacket raw;
        ...
        raw.data.assign(packet, packet + pkthdr->caplen);
    ...
    bool process_live(const std::string& device_name, const std::string& output_file) {
        ...
        if (device_name == "default" || device_name.empty()) {
            ...
            std::cout << "[Live] Opening default device: " << dev << "\n";
            handle = pcap_open_live(dev, 65535, 1, 1000, errbuf);
        } else {
            std::cout << "[Live] Opening device: " << device_name << "\n";
            handle = pcap_open_live(device_name.c_str(), 65535, 1, 1000, errbuf);
        }
        ...
        auto callback = [](u_char* user, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
            DPIEngine* engine = reinterpret_cast<DPIEngine*>(user);
            engine->handle_live_packet(pkthdr, packet);
        };

        std::cout << "[Live] Listening for packets. Press Ctrl+C to stop...\n";
...
int main(int argc, char* argv[]) {
    ...
    int arg_idx = 1;
    if (std::string(argv[arg_idx]) == "--live") {
        is_live = true;
        arg_idx++;
        if (arg_idx < argc) device_name = argv[arg_idx++];
        if (arg_idx < argc) output = argv[arg_idx++];
    } else {
        input = argv[arg_idx++];
        if (arg_idx < argc) output = argv[arg_idx++];
    }
    ...
    if (is_live) {
        if (!engine.process_live(device_name, output)) return 1;
    } else {
